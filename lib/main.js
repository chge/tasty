...
class Tasty {
	...
	constructor(config) {
		...
		config.runnerReporterOptions = config['runner-reporter-options'] ||
			config.runnerReporterOptions;
		...
		if (!config.url || config.url === true) {
			...
			config.url = config.url.endsWith('/') ?
				config.url :
				config.url + '/';
			config.url = Object.assign(
				parseUrl(url),
				parseUrl(config.url, false, true)
			);
		}
