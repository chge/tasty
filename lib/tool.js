...
exec(
	...
	/**
	 * (Re)sets or removes hook(s).
	 * @function hook
	 * @param {string[]} filter=[...] List of hook names.
	 ...
	 * @see {@link #exec|exec}
	 */
	complexTool('hook', (filter, fn, args, persistent) => {
		// TODO support '*' with excludes as '!...';
	...
	complexTool('ready', function(method, value, before, after, persistent) {
		...
		after = Array.isArray(after) ?
			...
			after ?
				...
				[
					'click',
					'connect', // NOTE instead of history, navigate and reload.
					'dblclick',
					'exec',
					'paste',
					'ready',
