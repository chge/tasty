...
class Server {
					...
					promise = urls ?
						urls.reduce(
							(chain, url) => chain.then(
								() => this.send(token, 'exec', url.slice(1))
							),
							Promise.resolve()
						) :
						Promise.resolve();
	...
	send(token, name, data) {
		// WORKAROUND: we use socket.io rooms to handle reconnects reliably.
		return new Promise((resolve, reject) => {
		...
		return Promise.resolve()
			.then(() => {
				if (this.coverage && config.coverageReporter) {
					this.log.log('client', token, 'coverage', config.coverageReporter);
	...
	onConnection(socket) {
		...
		socket.on('register', (token, callback) => {
			...
			if (this.coverage && config.coverageReporter) {
				socket.on('coverage', (data, callback) => {
					this.log.debug('client', token, 'coverage');
