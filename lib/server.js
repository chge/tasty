...
class Server {
	...
	start(id) {
		...
		return Promise.resolve()
			...
			.then(
				// WORKAROUND: drop reconnection state.
				() => this.send(id, 'noop', null)
			)
			.then(() => {
				if (runner.running) {
	...
	exec(id, code, args, persistent) {
		...
		scripts[path] = util.formatExec(path, code, values);
	...
	serveStatic(request, response) {
		...
		fs.access(path, fs.R_OK, (error) => {
			...
			if (this.coverage && mime(path) === mime.js) {
				...
				debug(200, url, 'from', path || '(invalid path)');
