...
`
	...
	serveFile(url, path, request, response) {
		...
		if (type === mime.html) {
			const href = config.url.href;
			readFile(path)
				.then((content) => {
					if (config.embed) {
						content = content.toString()
							.replace(`connect-src `, `connect-src ${href.replace(/^http/, 'ws')}`)
							.replace(`script-src `, `script-src ${href}`)
							.replace('<head>', `<head><script src="${href}tasty.js"></script>`);
					}
