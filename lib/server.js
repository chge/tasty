...
class Server {
	...
	close() {
		return new Promise((resolve, reject) => {
			...
			this.server.close((error) => {
				error ?
					this.log.warn('server', 'stopped', error) :
	...
	initRunner(client) {
		...
		const runner = createRunner(id, files, this, this.emitter, config, client.flaws);
		runner.id = id;
		runner.onTest = (name) => {
	...
	onRequest(request, response) {
		try {
			this.log.debug('server', request.method, request.headers.host || '(no host)', request.url);
	...
	onConnection(client) {
		...
		util.describeFlaws(flaws).forEach(
			(description) => old ||
				this.log.warn('client', id, description)
		);
	...
	serveStatic(request, response) {
		...
		fs.access(path, fs.R_OK, (error) => {
			...
			if (config.coverage && mime(path) === mime.js) {
				...
				this.log.debug('static', 200, url, 'from', path || '(invalid path)');
...
`
	...
	serveForbidden(url, path, response) {
		this.log.debug('server', 403, url, 'as', path);
	...
	serveNotFound(url, path, response) {
		this.log.debug('server', 404, url, 'as', path);
	...
	serveError(error, response) {
		this.log.debug('server', 500, error);
